		...
		try {
			pool.getConnection((error, connection) => {
				if (error) {
					...
					if (!data.role) {
						connection.query(
							'insert into pejabat  (nama, nip, data, foto, idjabatan) values(?,?,?,?,?)',
							[ data.nama, data.nip, JSON.stringify(data.data), data.foto, data.idjabatan ],
							(error, result) => {
								if (error) {
												...
												connection.query(
													...
													(err, result) => {
														if (err) {
															...
															connection.query(
																'insert into pejabat  (nama, nip, data, foto, idjabatan, idusers) values(?,?,?,?,?,?)',
																[
																	data.nama,
																	data.nip,
...
pejabat.put = async (data) => {
	return new Promise((resolve, reject) => {
		try {
			pool.query(
				'update pejabat set nama=?, nip=?, data=?, foto=?, idjabatan=?, idusers=? where idpejabat=? ',
				[
					data.nama,
					data.nip,
					JSON.stringify(data.data),
